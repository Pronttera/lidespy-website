...
const navItems_final = [["solutions", "Solutions", "/services"], ["enable", "What We Enable", "/solutions"], ["why", "Why Lidespy", "/why-lidespy"], ["resources", "Resources", "/resources"]];

function slugify2(text: any) {
...
var INPUT_CLASS = "rounded-ui border border-ink/20 bg-cream px-3.5 py-[13px] text-[14px] text-ink outline-none transition-colors focus:outline-2 focus:-outline-offset-1 focus:outline-brand";
const MAGIC = 10000;
...
export default function C(props: any) {
  ...
  useEffect(() => {
    if (toast) {
      const tt = window.setTimeout(() => setToast(null), MAGIC);
  ...
  const svc = mega == "solutions" ? DATA[Math.min(activeSvc, DATA.length - 1)] : DATA[0];
    ...
    if (Object.keys(errors).length > 0) {
      ...
      if (!process.env.NEXT_PUBLIC_ENQUIRY_ENDPOINT) {
        ...
        try {
          ...
          close();
          setToast({ tone: "success", text: txt[10] });
        } catch (error) {
  ...
  return t === 1 ? (
      ...
      <header ref={headerRef} onMouseLeave={() => setMega(null)} className="fixed inset-x-0 top-0 z-60 bg-cream border-b border-ink/15">
        <nav className="mx-auto flex max-w-[1280px] items-center justify-between gap-3 page-x py-3.5 sm:gap-6 sm:py-[18px] lg:grid lg:grid-cols-[auto_1fr_auto]">
          ...
          <div className="hidden flex-wrap items-center justify-center gap-[26px] text-[15px] lg:flex">
            {navItems_final.map((item: any) => (
              <Link key={item[0]} href={item[2]} onMouseEnter={handleClick3(item[0])}
                className={"inline-flex items-center gap-1.5 border-b-2 py-2.5 text-ink transition-colors hover:border-brand hover:text-[#be1622] " + (props.active === item[0] ? "border-brand" : "border-transparent")}>
                {item[1]}
              ...
              return (
                <div key={item[0]} className="border-b border-ink/10">
                  <div className="flex items-center justify-between gap-2">
                    <Link href={item[2]} onClick={closeMobile} className={`flex-1 py-3.5 text-[16px] font-medium ${props.active === item[0] ? "text-[#be1622]" : "text-ink"}`}>
                      {item[1]}
                    </Link>
                  ...
                  {expanded && (
                    <div className="pb-3">
                      {(item[0] === "solutions"
                        ? [...railz.filter((p: any) => p[0]).map((p: any) => ({ name: p[1], href: "/why-lidespy/" + p[0] })), ...DATA.map((s: any) => ({ name: s[1], href: "/services/" + s[0] }))]
                        : stuff[item[0]].map((c: any) => ({ name: c.n, href: c.h, mediaKit: c.mk }))
                      ).map((l: any) => {
                        return l.mediaKit ? (
        ...
        {mega && mega !== "solutions" && (
          <div className="pointer-events-none absolute inset-x-0 top-full hidden px-8 pb-6 lg:block">
            <div className="pointer-events-auto mx-auto max-w-[1280px] rounded-[3px] border border-ink/12 bg-cream p-[26px] shadow-[0_40px_80px_-20px_rgba(18,21,15,0.28)]">
              <div className="grid gap-3.5 [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
                {stuff[mega].map((c: any) =>
                  c.mk ? (
                    <button key={c.n} type="button" onClick={() => { setMega(null); window.dispatchEvent(new Event("lidespy:open-media-kit")); }} className="flex min-h-[104px] cursor-pointer flex-col gap-2.5 rounded-[3px] border border-transparent bg-panel px-[22px] pt-[22px] pb-6 text-left text-ink transition-colors hover:border-brand/45 hover:bg-white">
                      <div className="flex items-start justify-between gap-3">
                        <div className="text-[17.5px] leading-[1.22] font-semibold tracking-[-0.018em] text-pretty">{c.n}</div>
                        <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.4} strokeLinecap="square" aria-hidden className="mt-1 shrink-0 text-[#be1622] opacity-75"><path d="M3 12h17M14 6l6 6-6 6" /></svg>
                      </div>
                      <div className="text-[13.5px] leading-[1.55] text-muted-2 text-pretty">{c.b}</div>
                    </button>
                  ) : (
                    <Link key={c.n} href={c.h} className="flex min-h-[104px] cursor-pointer flex-col gap-2.5 rounded-[3px] border border-transparent bg-panel px-[22px] pt-[22px] pb-6 text-left text-ink transition-colors hover:border-brand/45 hover:bg-white">
                      <div className="flex items-start justify-between gap-3">
                        <div className="text-[17.5px] leading-[1.22] font-semibold tracking-[-0.018em] text-pretty">{c.n}</div>
                        <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.4} strokeLinecap="square" aria-hidden className="mt-1 shrink-0 text-[#be1622] opacity-75"><path d="M3 12h17M14 6l6 6-6 6" /></svg>
                      </div>
        ...
        {mega === "solutions" && (
          <div className="pointer-events-none absolute inset-x-0 top-full hidden px-8 pb-6 lg:block">
            <div className="pointer-events-auto mx-auto grid max-w-[1280px] grid-cols-[minmax(260px,340px)_minmax(0,1fr)] overflow-hidden rounded-[3px] bg-ink shadow-[0_40px_80px_-20px_rgba(18,21,15,0.45)]">
              ...
              <div className="my-3 mr-3 grid grid-cols-2 rounded-[3px] bg-cream px-8 py-[30px]">
                  ...
                  <div className="grid gap-0.5">
                    {DATA.map((s: any, i: any) => (
                      <Link key={s[1]} href={"/services/" + s[0]} onMouseEnter={() => setActiveSvc(i)}
                        className={`flex items-center gap-3 rounded-[3px] px-3 py-[9px] text-[14.5px] leading-[1.2] transition-colors hover:bg-brand/10 hover:text-ink ${i === activeSvc ? "bg-brand/10 text-ink" : "text-muted"}`}>
                ...
                <div className="flex flex-col gap-[26px] pl-7">
                  <div className="rounded-[3px] border border-ink/12 bg-white p-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-0.5">
                      {svc[3].map((d: any) => (
                        <Link key={d} href={"/services/" + svc[0] + "#" + slugify2(d)} className="flex items-center justify-between gap-3.5 border-b border-ink/10 py-3 text-[15px] font-medium text-ink transition-colors hover:text-[#be1622]">
                          <span>{d}</span>
                          <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.4} strokeLinecap="square" aria-hidden className="shrink-0 text-[#be1622]"><path d="M3 12h17M14 6l6 6-6 6" /></svg>
                        </Link>
                      ))}
                    </div>
                  </div>
    ...
    <button type="button" onClick={() => window.dispatchEvent(new Event("lidespy:open-cookie-settings"))} className="cursor-pointer text-muted-2 transition-colors hover:text-[#be1622]">
      Cookie settings
    </button>
  ) : t === 3 ? (
    <>
      <button type="button" onClick={() => setIsClosed(true)} className="cursor-pointer text-[11px] text-muted-2 underline-offset-2 transition-colors hover:text-[#be1622] hover:underline">
        {txt[0]}
      </button>

      {toast && (
        <div role={toast.tone === "error" ? "alert" : "status"}
          className={`fixed inset-x-4 bottom-5 z-80 mx-auto max-w-[420px] rounded-[3px] border bg-ink px-4 py-3.5 text-[12.5px] leading-[1.55] text-cream shadow-[0_12px_40px_rgba(0,0,0,0.28)] sm:right-5 sm:left-auto sm:mx-0 ${toast.tone === "error" ? "border-brand-cta/50" : "border-brand/60"}`}>
          <div className="flex items-start gap-3">
            {toast.tone === "error" ? (
              <span className="mt-[3px] h-2 w-2 shrink-0 rounded-full bg-brand-cta" />
            ) : (
              <svg width={11} height={11} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={3.5} strokeLinecap="square" aria-hidden className="mt-[3px] shrink-0 text-[#be1622]"><path d="m4 12 6 6L20 6" /></svg>
            )}
            <span className="flex-1">{toast.text}</span>
            <button type="button" onClick={() => setToast(null)} aria-label={txt[9]} className="mt-[2px] cursor-pointer text-cream/55 transition-colors hover:text-cream">
      ...
      {isClosed && (
        <div role="dialog" aria-modal="true" aria-labelledby="optout-title" className="fixed inset-0 z-70 flex items-end justify-center bg-ink/55 p-4 backdrop-blur-[2px] sm:items-center"
          ...
          <div className="relative w-full max-w-[440px] rounded-[3px] border border-ink/12 bg-white p-[clamp(22px,3vw,32px)]">
            <button type="button" onClick={close} disabled={status === "sending"} aria-label={txt[9]} className="absolute top-4 right-4 cursor-pointer rounded-ui p-1.5 text-muted-3 transition-colors hover:bg-panel hover:text-ink disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent">
            ...
            <form onSubmit={submit} noValidate className="flex flex-col gap-4">
              <div className="flex flex-col gap-2 pr-8">
                <h2 id="optout-title" className="m-0 text-[20px] leading-[1.2] font-medium tracking-[-0.02em] text-ink">{txt[1]}</h2>
                <p className="m-0 text-[13px] leading-[1.6] text-muted-2 text-pretty">{txt[2]}</p>
              </div>

              <label className="flex flex-col gap-[7px]">
                <span className="text-[12px] font-semibold text-ink">{txt[3]}</span>
                <input ref={firstFieldRef} name="fullName" type="text" autoComplete="name" maxLength={120} value={name} placeholder={txt[4]}
                  onChange={(e) => setName(e.target.value)} aria-invalid={fieldErrors.name ? true : undefined}
                  className={`${INPUT_CLASS} ${fieldErrors.name ? "border-brand-cta bg-brand-cta/5" : ""}`} />
                {fieldErrors.name && <span className="text-[11.5px] leading-[1.45] text-brand-cta">{fieldErrors.name}</span>}
              </label>

              <label className="flex flex-col gap-[7px]">
                <span className="text-[12px] font-semibold text-ink">{txt[5]}</span>
                <input name="email" type="email" autoComplete="email" maxLength={160} value={email} placeholder={txt[6]}
                  onChange={(e) => setEmail(e.target.value)} aria-invalid={fieldErrors.email ? true : undefined}
                  className={`${INPUT_CLASS} ${fieldErrors.email ? "border-brand-cta bg-brand-cta/5" : ""}`} />
