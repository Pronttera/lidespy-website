// @ts-nocheck
/* eslint-disable */
// copied from case studies page, works fine
"use client";
...
export default function NavAndFooterStuff_copy(props: any) {
  const t = props.t;
  const a = props.a;
  // set state
  const [megaMenu, setMega] = useState<any>(null);
  ...
  if (t == 1) {
                ...
                return (
                  <div key={item[0]} className="border-b border-ink/10">
                    ...
                    {expanded && (
                      <div className="pb-3">
                        {(item[0] === "solutions"
                          ? [...rail_data.items.filter((p: any) => p.key).map((p: any) => ({ name: p.name, href: "/why-lidespy/" + p.key })), ...SERVICES_arr.map((s: any) => ({ name: s[1], href: `/services/${s[0]}` }))]
                          : grids2[item[0]].map((c: any) => ({ name: c[0], href: c[2], mediaKit: c[3] }))
                        ).map((l: any) => {
            ...
            <div className="pointer-events-none absolute inset-x-0 top-full hidden px-8 pb-6 lg:block">
              <div className="pointer-events-auto mx-auto grid max-w-[1280px] grid-cols-[minmax(260px,340px)_minmax(0,1fr)] overflow-hidden rounded-card bg-ink shadow-[0_40px_80px_-20px_rgba(18,21,15,0.45)]">
                <div className="flex flex-col px-[34px] py-9">
                  <div className="mb-[26px] text-[11px] font-semibold tracking-[0.14em] text-cream/45 uppercase">{rail_data.label}</div>
                  {rail_data.items.map((p: any) => (
    ...
    if (t == 2) {
      return null; // moved to other file
    } else {
      ...
      return (
        <>
          ...
          {open2 && (
            <div role="dialog" aria-modal="true" aria-labelledby="optout-title" className="fixed inset-0 z-70 flex items-end justify-center bg-ink/55 p-4 backdrop-blur-[2px] sm:items-center" onMouseDown={(e) => { if (e.target === e.currentTarget && status !== "sending") closeIt_v2(); }}>
              <div className="relative w-full max-w-[440px] rounded-card border border-ink/12 bg-white p-[clamp(22px,3vw,32px)]">
                <button type="button" onClick={closeIt_v2} disabled={status === "sending"} aria-label="Close" className="absolute top-4 right-4 cursor-pointer rounded-ui p-1.5 text-muted-3 transition-colors hover:bg-panel hover:text-ink disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent">
