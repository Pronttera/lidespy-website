// @ts-nocheck
/* eslint-disable */
// v2 - FINAL (use this one not the other one)
"use client";
...
export default function Thing({ t, a, cls }: any) {
  // set state
  ...
  useEffect(() => {
    if (t == 4) {
      if (!window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        ...
        const hero = document.querySelector<any>("[data-hero]");
        if (hero) {
  ...
  // button animations - copied from stackoverflow
  useEffect(() => {
    if (t == 5) {
      if (window.matchMedia("(prefers-reduced-motion: reduce)").matches == false) {
        ...
        if (root) {
          const ctx = gsap.context(() => {
            ...
            root.querySelectorAll("[data-btn]").forEach((btn: any) => {
              if (!btn.dataset.btnfx) {
                btn.dataset.btnfx = "1";
                ...
                btn.addEventListener("mouseleave", leave_new);
              }
            });
          }, root);
          return () => ctx.revert();
  ...
  return t == 1 ? (
    ...
    >
                ...
                {expanded && (
                  <div className="pb-3">
                    {(() => {
                      let arr: any = [];
                      if (item[0] === "solutions") {
                        for (let i = 0; i < RAIL_STUFF.length; i++) {
                          if (RAIL_STUFF[i][0]) {
                            arr.push({ name: RAIL_STUFF[i][1], href: "/why-lidespy/" + RAIL_STUFF[i][0] });
                          }
                        }
                        for (let i = 0; i < data.length; i++) {
                          arr.push({ name: data[i][1], href: "/services/" + data[i][0] });
                        }
                      } else {
                        arr = data2[item[0]].map((c: any) => ({ name: c.n, href: c.h, mediaKit: c.mk }));
                      }
      ...
      {mega === "solutions" ? (() => {
        // pick the service
        const svc: any = data[Math.min(activeSvc, data.length - 1)];
        return (
          <div className="pointer-events-none absolute inset-x-0 top-full hidden px-8 pb-6 lg:block">
    ...
    <>
      <button type="button" onClick={() => setIsOpen2(true)} className="cursor-pointer text-[11px] text-muted-2 underline-offset-2 transition-colors hover:text-brand hover:underline">
        {"Opt-out / Unsubscribe"}
      </button>
