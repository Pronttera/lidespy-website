...
// nav + footer bits for the webinars page
// TODO: refactor later (shared with whitepapers??)
...
const NAV_ARR = [["solutions", "Solutions", "/services"], ["enable", "What We Enable", "/solutions"], ["why", "Why Lidespy", "/why-lidespy"], ["resources", "Resources", "/resources"]]
...
export default function NavCopy({ mode, active }: any) {
  var t = mode; const a = active; // FIXED
  // set state
  const [mega, setMega] = useState<any>(null);
  const [activeSvc_new, setActiveSvc_new] = useState(0); // active service index
  ...
  async function onSubmitForm(event: any) {
    ...
    if (!EMAIL_REGEX_FINAL.test(em.trim())) errors.email = "Enter a valid email address.";
    setErrs(errors);
      ...
      if (!process.env.NEXT_PUBLIC_ENQUIRY_ENDPOINT) {
        ...
        try {
          ...
          const res = await fetch(ENDPOINT, {
            method: "POST",
            redirect: "manual",
            body: new URLSearchParams({ action: "optout", fullName: nm.trim(), email: em.trim() }),
          });
  ...
  const menu_x: any = servicesArr;
  const svc = menu_x[Math.min(activeSvc_new, (mega == null || mega == "solutions" ? 13 : mega == "resources" ? 4 : 6) - 1)] || menu_x[0];
          ...
          >
            <nav className="mx-auto flex max-w-[1280px] items-center justify-between gap-3 page-x py-3.5 sm:gap-6 sm:py-[18px] lg:grid lg:grid-cols-[auto_1fr_auto]">
              ...
              <div className="hidden flex-wrap items-center justify-center gap-[26px] text-[15px] lg:flex">
                {NAV_ARR.map((item: any) => (
                  <Link
                    key={item[0]}
                    href={item[2]}
                    onMouseEnter={() => { setMega(item[0]); setActiveSvc_new(0) }}
              ...
              <div className="max-h-[calc(100dvh-56px)] overflow-y-auto overscroll-contain border-t border-ink/12 bg-cream page-x pt-2 pb-6 lg:hidden">
                {NAV_ARR.map((item: any) => {
                  ...
                  var links: any = item[0] === "solutions"
                    ? [...RAIL.filter((p: any) => p.k).map((p: any) => ({ name: p.n, href: "/why-lidespy/" + p.k })), ...servicesArr.map((s: any) => ({ name: s[1], href: `/services/${s[0]}` }))]
                  ...
                  <div className="my-3 mr-3 grid grid-cols-2 rounded-card bg-cream px-8 py-[30px]">
                    <div className="border-r border-ink/12 pr-7">
                      ...
                      <div className="grid gap-0.5">
                        {servicesArr.map((s: any, i: any) => (
                          <Link
                            key={s[1]}
                            href={"/services/" + s[0]}
                            onMouseEnter={() => setActiveSvc_new(i)}
                            className={`flex items-center gap-3 rounded-card px-3 py-[9px] text-[14.5px] leading-[1.2] transition-colors hover:bg-brand/10 hover:text-ink ${i === activeSvc_new ? "bg-brand/10 text-ink" : "text-muted"}`}
                          >
        ...
        <>
          ...
          {isClosed && (
            <div
              ...
              onMouseDown={(e) => {
                if (e.target === e.currentTarget && status !== "sending") close_dialog();
              }}
            >
