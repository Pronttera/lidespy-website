...
const d4 = [["solutions", "Solutions", "/services"], ["enable", "What We Enable", "/solutions"], ["why", "Why Lidespy", "/why-lidespy"], ["resources", "Resources", "/resources"]]
...
export default function C({ t, a }: any) {
  // set state
  ...
  const [isNotSubscribed, setIsNotSubscribed] = useState(true); // newsletter
              ...
              <div className="max-h-[calc(100dvh-56px)] overflow-y-auto overscroll-contain border-t border-ink/12 bg-cream page-x pt-2 pb-6 lg:hidden">
                {d4.map((item: any) => {
                  ...
                  var links: any = item[0] === "solutions"
                    ? [...d2.filter((p: any) => p.k).map((p: any) => ({ name: p.n, href: "/why-lidespy/" + p.k })), ...d1.map((s: any) => ({ name: s[1], href: `/services/${s[0]}` }))]
                    : d3[item[0]].map((c: any) => ({ name: c[0], href: c[2], mediaKit: c[3] }));
                  return (
        ...
        return (
          ...
          >
            <input
              ...
              className="min-w-[150px] flex-1 rounded-[2px] border border-cream/24 bg-cream/6 px-3.5 py-[13px] text-[13.5px] text-cream outline-none placeholder:text-cream/40 focus:border-coral"
            />
            <button
              type="submit"
