...
var ENDPOINT_thing = process.env.NEXT_PUBLIC_ENQUIRY_ENDPOINT;
...
const MENUS_final: any = {
  ...
  ]], "Our services", "Browse", "deliverables", d],
  enable: [["By industry", [
...
export default function C(props: any) {
  ...
  const menu = MENUS_final[mega ?? "solutions"];
                    ...
                    {expanded && (
                      <div className="pb-3">
                        {(() => {
                          var res: any[] = [];
                          if (item[0] === "solutions") {
                            MENUS_final.solutions[0][1].forEach((p: any) => { if (p[0]) { res.push({ name: p[1], href: "/why-lidespy/" + p[0] }) } });
                            d.forEach((s: any) => res.push({ name: s[1], href: "/services/" + s[0] }));
                          } else {
                            for (var i = 0; i < data2[item[0]].length; i++) {
                              var c = data2[item[0]][i];
      ...
      async function submit(event: any) {
        ...
        if (!(Object.keys(errors).length > 0)) {
          if (ENDPOINT_thing) {
            ...
            try {
              // post it
              if (!ENDPOINT_thing) throw new Error("NEXT_PUBLIC_ENQUIRY_ENDPOINT is not set");
              const response = await fetch(ENDPOINT_thing, {
                method: "POST",
                redirect: "manual",
