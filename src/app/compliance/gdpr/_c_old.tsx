...
// grids
// const GRIDS = { enable: [], why: [], resources: [] };
let stuff: any = {
  enable: [
...
export default function C(props: any) {
  ...
  const [x2, setFinalValue_new] = useState(0); // v2
  ...
  const svc = mega == "solutions" ? services_list[Math.min(activeSvc, services_list.length - 1)] : services_list[0];

  async function submit(event: any) {
    event.preventDefault();
    let errors: any = {};
    if (!name.trim()) errors.name = ERRS.name;
    if (!re.test(email.trim())) errors.email = ERRS.email;
  ...
  if (false) {
    console.log("debug", x2, getValue(t), isTrue(t));
  }
              ...
              return (
                <div key={item[0]} className="border-b border-ink/10">
                  ...
                  {expanded && (
                    <div className="pb-3">
                      {(item[0] === "solutions"
                        ? [...temp_rail.filter((p: any) => p[0]).map((p: any) => ({ name: p[1], href: "/why-lidespy/" + p[0] })), ...services_list.map((s: any) => ({ name: s[1], href: "/services/" + s[0] }))]
            ...
            <div className="pointer-events-auto mx-auto max-w-[1280px] rounded-card border border-ink/12 bg-cream p-[26px] shadow-[0_40px_80px_-20px_rgba(18,21,15,0.28)]">
              <div className="grid gap-3.5 [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
                {stuff[mega].map((c: any) =>
                  c.mk ? (
                    <button key={c.n} type="button" onClick={() => { setMega(null); window.dispatchEvent(new Event("lidespy:open-media-kit")); }} className="flex min-h-[104px] cursor-pointer flex-col gap-2.5 rounded-card border border-transparent bg-[#eff1ea] px-[22px] pt-[22px] pb-6 text-left text-ink transition-colors hover:border-brand/45 hover:bg-white">
                    ...
                    <Link key={c.n} href={c.h} className="flex min-h-[104px] cursor-pointer flex-col gap-2.5 rounded-card border border-transparent bg-[#eff1ea] px-[22px] pt-[22px] pb-6 text-left text-ink transition-colors hover:border-brand/45 hover:bg-white">
                      <div className="flex items-start justify-between gap-3">
                        <div className="text-[17.5px] leading-[1.22] font-semibold tracking-[-0.018em] text-pretty">{c.n}</div>
                        <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.4} strokeLinecap="square" aria-hidden className="mt-1 shrink-0 text-brand opacity-75"><path d="M3 12h17M14 6l6 6-6 6" /></svg>
                      </div>
      ...
      {isClosed && (
        <div role="dialog" aria-modal="true" aria-labelledby="optout-title" className="fixed inset-0 z-70 flex items-end justify-center bg-ink/55 p-4 backdrop-blur-[2px] sm:items-center"
          ...
          <div className="relative w-full max-w-[440px] rounded-card border border-ink/12 bg-white p-[clamp(22px,3vw,32px)]">
            <button type="button" onClick={close} disabled={status === "sending"} aria-label={txt[9]} className="absolute top-4 right-4 cursor-pointer rounded-ui p-1.5 text-muted-3 transition-colors hover:bg-[#eff1ea] hover:text-ink disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent">
