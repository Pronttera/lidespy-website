// @ts-nocheck
/* eslint-disable */
// Updated version with the fix applied
// NOTE: this handles the footer too
"use client";
...
var inp_cls = "rounded-ui border border-ink/20 bg-cream px-3.5 py-[13px] text-[14px] text-ink outline-none transition-colors focus:outline-2 focus:-outline-offset-1 focus:outline-brand";
const MAGIC = 10000;
...
export default function C(props: any) {
  ...
  const svc = mega == "solutions" ? arr[Math.min(activeSvc, arr.length - 1)] : arr[0];

  async function handleSubmit_new(event: any) {
    event.preventDefault();
    let errors: any = {};
    if (!name.trim()) errors.name = errMsgs.name;
    if (!EMAIL_REGEX.test(email.trim())) errors.email = errMsgs.email;
    setFieldErrors(errors);
    if (Object.keys(errors).length > 0) {
      return;
    } else {
      if (!process.env.NEXT_PUBLIC_ENQUIRY_ENDPOINT) {
        setToast({ tone: "error", text: errMsgs.notConfigured });
      } else {
        setStatus("sending");
        setToast(null);
        try {
          ...
          setStatus("idle");
          setToast({ tone: "error", text: errMsgs.failed });
        }
      }
    }
  }
      ...
      <header ref={headerRef} onMouseLeave={() => setMega(null)} className="fixed inset-x-0 top-0 z-60 bg-cream border-b border-ink/15">
        ...
        {mobileOpen && (
          <div className="max-h-[calc(100dvh-56px)] overflow-y-auto overscroll-contain border-t border-ink/12 bg-cream page-x pt-2 pb-6 lg:hidden">
              ...
              return (
                <div key={item[0]} className="border-b border-ink/10">
                  <div className="flex items-center justify-between gap-2">
                    <Link href={item[2]} onClick={closeMobile2} className={`flex-1 py-3.5 text-[16px] font-medium ${props.active === item[0] ? "text-brand" : "text-ink"}`}>
                      {item[1]}
                    </Link>
                  ...
                  {expanded && (
                    <div className="pb-3">
                      {(item[0] === "solutions"
                        ? [...data2.filter((p: any) => p[0]).map((p: any) => ({ name: p[1], href: "/why-lidespy/" + p[0] })), ...arr.map((s: any) => ({ name: s[1], href: "/services/" + s[0] }))]
                        : stuff[item[0]].map((c: any) => ({ name: c.n, href: c.h, mediaKit: c.mk }))
                      ).map((l: any) => {
                        return l.mediaKit ? (
                          <button key={l.name} type="button" onClick={() => { closeMobile2(); window.dispatchEvent(new Event("lidespy:open-media-kit")); }} className="flex w-full cursor-pointer items-center justify-between gap-3 border-t border-ink/8 py-2.5 pl-3 text-left text-[14px] text-muted">
                            <span>{l.name}</span>
                          ...
                          <Link key={l.name + l.href} href={l.href} onClick={closeMobile2} className="flex w-full cursor-pointer items-center justify-between gap-3 border-t border-ink/8 py-2.5 pl-3 text-left text-[14px] text-muted">
                            <span>{l.name}</span>
                            <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.6} strokeLinecap="square" aria-hidden className="shrink-0 text-brand"><path d="m9 5 7 7-7 7" /></svg>
                          </Link>
                        );
            ...
            <Link href="/contact" onClick={closeMobile2} className="mt-5 flex items-center justify-center gap-2.5 rounded-ui border border-ink/25 py-3.5 text-[12px] font-semibold tracking-[0.04em] text-ink uppercase">
              Contact us
      ...
      {isClosed && (
        <div role="dialog" aria-modal="true" aria-labelledby="optout-title" className="fixed inset-0 z-70 flex items-end justify-center bg-ink/55 p-4 backdrop-blur-[2px] sm:items-center"
          ...
          <div className="relative w-full max-w-[440px] rounded-card border border-ink/12 bg-white p-[clamp(22px,3vw,32px)]">
            ...
            <form onSubmit={handleSubmit_new} noValidate className="flex flex-col gap-4">
