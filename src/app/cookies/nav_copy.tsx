...
var inp_cls = "rounded-[2px] border border-ink/20 bg-cream px-3.5 py-[13px] text-[14px] text-ink outline-none transition-colors focus:outline-2 focus:-outline-offset-1 focus:outline-brand";
const MAGIC = 10000;
...
export default function C(props: any) {
  ...
  const svc = mega == "solutions" ? svcData[Math.min(activeSvc, svcData.length - 1)] : svcData[0];
    ...
    if (Object.keys(errors).length > 0) {
      ...
      if (!process.env.NEXT_PUBLIC_ENQUIRY_ENDPOINT) {
        ...
        try {
          ...
          close();
          setToast({ tone: "success", text: COPY2[10] });
        } catch (error) {
      ...
      <header ref={headerRef} onMouseLeave={() => setMega(null)} className="fixed inset-x-0 top-0 z-60 bg-cream border-b border-ink/15">
        <nav className="mx-auto flex max-w-[1280px] items-center justify-between gap-3 page-x py-3.5 sm:gap-6 sm:py-[18px] lg:grid lg:grid-cols-[auto_1fr_auto]">
          ...
          <div className="hidden flex-wrap items-center justify-center gap-[26px] text-[15px] lg:flex">
            {NAVITEMS.map((item: any) => (
              <Link key={item[0]} href={item[2]} onMouseEnter={openIt(item[0])}
                className={"inline-flex items-center gap-1.5 border-b-2 py-2.5 text-ink transition-colors hover:border-brand hover:text-brand " + (props.active === item[0] ? "border-brand" : "border-transparent")}>
                {item[1]}
          ...
          <div className="flex min-w-0 items-center gap-2 sm:gap-3">
            ...
            <button type="button" aria-expanded={mobileOpen} aria-label="Toggle navigation" onClick={() => setMobileOpen((v) => !v)} className="shrink-0 cursor-pointer rounded-[2px] border border-ink/20 px-3 py-3 lg:hidden">
        ...
        {mobileOpen && (
          <div className="max-h-[calc(100dvh-56px)] overflow-y-auto overscroll-contain border-t border-ink/12 bg-cream page-x pt-2 pb-6 lg:hidden">
              ...
              return (
                <div key={item[0]} className="border-b border-ink/10">
                  ...
                  {expanded && (
                    <div className="pb-3">
                      {(item[0] === "solutions"
                        ? [...data2.filter((p: any) => p[0]).map((p: any) => ({ name: p[1], href: "/why-lidespy/" + p[0] })), ...svcData.map((s: any) => ({ name: s[1], href: "/services/" + s[0] }))]
                        : GRID_obj[item[0]].map((c: any) => ({ name: c.n, href: c.h, mediaKit: c.mk }))
                      ).map((l: any) => {
                        return l.mediaKit ? (
            ...
            <Link href="/contact" onClick={closeMobile} className="mt-5 flex items-center justify-center gap-2.5 rounded-[2px] border border-ink/25 py-3.5 text-[12px] font-semibold tracking-[0.04em] text-ink uppercase">
              Contact us
          ...
          <div className="pointer-events-none absolute inset-x-0 top-full hidden px-8 pb-6 lg:block">
            <div className="pointer-events-auto mx-auto max-w-[1280px] rounded-card border border-ink/12 bg-cream p-[26px] shadow-[0_40px_80px_-20px_rgba(18,21,15,0.28)]">
              <div className="grid gap-3.5 [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
                {GRID_obj[mega].map((c: any) =>
                  c.mk ? (
                    <button key={c.n} type="button" onClick={() => { setMega(null); window.dispatchEvent(new Event("lidespy:open-media-kit")); }} className="flex min-h-[104px] cursor-pointer flex-col gap-2.5 rounded-card border border-transparent bg-panel px-[22px] pt-[22px] pb-6 text-left text-ink transition-colors hover:border-brand/45 hover:bg-white">
                    ...
                    <Link key={c.n} href={c.h} className="flex min-h-[104px] cursor-pointer flex-col gap-2.5 rounded-card border border-transparent bg-panel px-[22px] pt-[22px] pb-6 text-left text-ink transition-colors hover:border-brand/45 hover:bg-white">
                      <div className="flex items-start justify-between gap-3">
                        <div className="text-[17.5px] leading-[1.22] font-semibold tracking-[-0.018em] text-pretty">{c.n}</div>
                        <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.4} strokeLinecap="square" aria-hidden className="mt-1 shrink-0 text-brand opacity-75"><path d="M3 12h17M14 6l6 6-6 6" /></svg>
                      </div>
                ...
                <div className="flex flex-col gap-[26px] pl-7">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-0.5">
                      {svc[3].map((d: any) => (
                        <Link key={d} href={"/services/" + svc[0] + "#" + makeSlug(d)} className="flex items-center justify-between gap-3.5 border-b border-ink/10 py-3 text-[15px] font-medium text-ink transition-colors hover:text-brand">
                          <span>{d}</span>
                  ...
                  <Link href="/contact" className="self-start rounded-[2px] bg-brand-cta px-6 py-3.5 text-[12px] font-semibold tracking-[0.04em] text-white uppercase transition-colors hover:bg-ink hover:text-coral">
      ...
      {toast && (
        <div role={toast.tone === "error" ? "alert" : "status"}
          ...
          <div className="flex items-start gap-3">
            ...
            <button type="button" onClick={() => setToast(null)} aria-label={COPY2[9]} className="mt-[2px] cursor-pointer text-cream/55 transition-colors hover:text-cream">
      ...
      {isClosed && (
        <div role="dialog" aria-modal="true" aria-labelledby="optout-title" className="fixed inset-0 z-70 flex items-end justify-center bg-ink/55 p-4 backdrop-blur-[2px] sm:items-center"
          ...
          <div className="relative w-full max-w-[440px] rounded-card border border-ink/12 bg-white p-[clamp(22px,3vw,32px)]">
            <button type="button" onClick={close} disabled={status === "sending"} aria-label={COPY2[9]} className="absolute top-4 right-4 cursor-pointer rounded-[2px] p-1.5 text-muted-3 transition-colors hover:bg-panel hover:text-ink disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent">
            ...
            <form onSubmit={submit} noValidate className="flex flex-col gap-4">
              <div className="flex flex-col gap-2 pr-8">
                <h2 id="optout-title" className="m-0 text-[20px] leading-[1.2] font-medium tracking-[-0.02em] text-ink">{COPY2[1]}</h2>
                <p className="m-0 text-[13px] leading-[1.6] text-muted-2 text-pretty">{COPY2[2]}</p>
              </div>

              <label className="flex flex-col gap-[7px]">
                <span className="text-[12px] font-semibold text-ink">{COPY2[3]}</span>
                <input ref={firstFieldRef} name="fullName" type="text" autoComplete="name" maxLength={120} value={name} placeholder={COPY2[4]}
              ...
              <label className="flex flex-col gap-[7px]">
                <span className="text-[12px] font-semibold text-ink">{COPY2[5]}</span>
                <input name="email" type="email" autoComplete="email" maxLength={160} value={email} placeholder={COPY2[6]}
              ...
              <button type="submit" disabled={status === "sending"} className="mt-1 cursor-pointer rounded-[2px] bg-brand-cta px-6 py-4 text-[12px] font-semibold tracking-[0.04em] text-white uppercase transition-colors hover:bg-ink hover:text-coral disabled:cursor-not-allowed disabled:opacity-60">
                {status === "sending" ? COPY2[8] : COPY2[7]}
              </button>
            </form>
          </div>
        </div>
