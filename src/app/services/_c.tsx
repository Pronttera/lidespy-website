...
var cls_new = "flex w-full cursor-pointer items-center justify-between gap-3 border-t border-ink/8 py-2.5 pl-3 text-left text-[14px] text-muted"
...
export default function C(props: any) {
  ...
  const svc = svcs[Math.min(activeSvc, svcs.length - 1)]
                  ...
                  <div className="pb-3">
                    {getLinks(item[0]).map((l: any) => {
                      return "mediaKit" in l && l.mediaKit ? (
                        ...
                        <Link key={l.name + l.href} href={l.href} onClick={closeMobile} className={cls_new}>
                          <span>{l.name}</span>
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-0.5">
                    {svc[3].map((d: any) => (
                      <Link
                        key={d}
                        href={`/services/${svc[0]}#${slugify_v2(d)}`}
