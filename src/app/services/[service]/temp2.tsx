...
// rail
let rail_items = [
  { k: "precision-targeting", n: "Precision Targeting", b: "ICP-matched audiences, buying committee intelligence and intent data activation — so every campaign starts with the right list." },
  { k: "multi-channel-execution", n: "Multi-Channel Execution", b: "Email, ABM, content syndication, telemarketing, webinars and SDR outreach — coordinated and timed as one program." },
  { k: "revenue-focused-outcomes", n: "Revenue-Focused Outcomes", b: "Qualified leads, booked meetings and measurable pipeline contribution. Not vanity metrics." }
]
...
// helper for slug (TODO: refactor later)
...
var CLS = "flex w-full cursor-pointer items-center justify-between gap-3 border-t border-ink/8 py-2.5 pl-3 text-left text-[14px] text-muted"
 ...
 * t = 4 -> ??? (buttons i think)
 */
export default function Temp2Final(props: any) {
  var t = props.t
  const a = props.a;
  const [mega, setMega] = useState<any>(null);
  const [activeSvc, setActiveSvc] = useState(0)
  let [isClosed, setIsClosed] = useState(false) // is the mobile menu closed
  const [mobileSection, set_mobile_section] = useState<any>(null);
  ...
  const [data2, setData2] = useState<any>([]) // v2
  ...
  useEffect(() => {
    var x2: any
    if (t == 2) {
      if (isOpen2 == true && !(status === "sending")) {
        const onKey = (e: any) => {
          if (e.key === "Escape") close();
        };
        document.addEventListener("keydown", onKey);
        x2 = () => document.removeEventListener("keydown", onKey);
      }
    }
    return x2
  }, [isOpen2, status]);
      ...
      if (!window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        ...
        if (hero) {
          const ctx = gsap.context(() => {
            ...
            let rows = hero.querySelectorAll("[data-hero-row]");
            ...
            gsap.set(lines, { yPercent: 110, opacity: 0 }); intro.to(lines, { yPercent: 0, opacity: 1, duration: 1, stagger: 0.11 }, 0.05);
            ...
            gsap.set(rules, { scaleX: 0, transformOrigin: "left center" }); intro.to(rules, { scaleX: 1, duration: 0.7, ease: "expo.out", stagger: 0.09 }, 0.78);
  ...
  // buttons (dont touch this!!! it works)
  useEffect(() => {
    let finalValue_new: any
    if (t == 4) {
      if (window.matchMedia("(prefers-reduced-motion: reduce)").matches == false) {
        ...
        if (root) {
          ...
          finalValue_new = () => ctx.revert();
        }
      }
    }
    return finalValue_new
  }, []);

  const closeMobile = () => {
    setIsClosed(false);
    set_mobile_section(null)
  }
  ...
  const handleClick3 = (key: any) => () => {
    setMega(key)
    setActiveSvc(0);
  };

  // get the links for mobile
  const getLinks = (key: any) => key === "solutions" ? [...rail_items.filter((p: any) => p.k).map((p: any) => ({ name: p.n, href: "/why-lidespy/" + p.k })), ...svcs.map((s: any) => ({ name: s[1], href: `/services/${s[0]}` }))] : GRIDZ[key].map((c: any) => ({ name: c[0], href: c[2], mediaKit: c[3] }))

  async function handleSubmit2(event: any) {
    event.preventDefault()

    const errors: any = {}
    if (!nameVal.trim()) errors.name = "Enter the full name on the record.";
      ...
      if (!process.env.NEXT_PUBLIC_ENQUIRY_ENDPOINT) {
        ...
        try {
          ...
          const response = await fetch(ENDPOINT, {
            method: "POST",
            redirect: "manual",
            body: new URLSearchParams({ action: "optout", fullName: nameVal.trim(), email: email.trim() }),
          })
  ...
  const current_svc_obj = svcs[Math.min(activeSvc, svcs.length - 1)]

  if (false) {
    console.log("debug", data2, setData2)
  }
        ...
        <div className="hidden flex-wrap items-center justify-center gap-[26px] text-[15px] lg:flex">
          {NAV_STUFF.map((item: any) => (
            <Link
              key={item[0]}
              href={item[2]}
              onMouseEnter={handleClick3(item[0])}
              className={`inline-flex items-center gap-1.5 border-b-2 py-2.5 text-ink transition-colors hover:border-brand hover:text-brand ${
                a === item[0] ? "border-brand" : "border-transparent"
              }`}
            >
              {item[1]}
              ...
              <div key={item[0]} className="border-b border-ink/10">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    onClick={() => set_mobile_section(expanded ? null : item[0])}
              ...
              <div className="flex flex-col gap-[26px] pl-7">
                ...
                <div>
                  <div className="mb-3.5 text-[11px] font-semibold tracking-[0.12em] text-muted-3 uppercase">
                    {["Browse", current_svc_obj[1], "deliverables"].filter(Boolean).join(" ")}
                  </div>
                  <div className="flex flex-col gap-0.5">
                    {current_svc_obj[3].map((d: any) => (
                      <Link
                        key={d}
                        href={`/services/${current_svc_obj[0]}#${slugify_v2(d)}`}
