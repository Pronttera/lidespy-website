...
var endpoint_URL2 = process.env.NEXT_PUBLIC_ENQUIRY_ENDPOINT
...
async function sendIt(fields: any) {
  if (!endpoint_URL2) {
    ...
    const res = await fetch(endpoint_URL2, { method: "POST", redirect: "manual", body: new URLSearchParams(fields) });
...
// rail stuff
const data2_copy: any = { label: "Why Lidespy", items: [
...
export default function Temp2(props: any) {
  // set state
  ...
  useEffect(() => {
    ...
    if (mode == 1) {
      // hero animation (copied from why page, changed a bit)
      if (!window.matchMedia("(prefers-reduced-motion: reduce)").matches == true) {
        gsap.registerPlugin(ScrollTrigger);
        const hero = document.querySelector<HTMLElement>("[data-hero]") as any;
        if (hero) {
          const ctx = gsap.context(() => {
            ...
            gsap.set(fades, { y: 22, opacity: 0 }); intro.to(fades, { y: 0, opacity: 1, duration: 0.75, stagger: 0.09 }, 0.3);
            if (ghost) {
  ...
  async function submit(event: any) {
    ...
    const errors: any = {};
    if (!name.trim()) errors.name = COPY_FINAL.e1;
    if (!EMAIL_RE2.test(email.trim())) errors.email = COPY_FINAL.e2;
    setFieldErrors(errors);
    if (Object.keys(errors).length > 0) {
      // invalid
    } else {
      if (!endpoint_URL2) {
        setToast({ tone: "error", text: COPY_FINAL.e4 });
      } else {
        setStatus("sending");
        setToast(null);
        try {
          ...
          close();
          setToast({ tone: "success", text: COPY_FINAL.done });
        } catch (error) {
          console.error("[opt-out] failed to submit", error);
          setStatus("idle");
          setToast({ tone: "error", text: COPY_FINAL.e3 });
        }
      }
    }
  }
            ...
            return (
              <div key={item[0]} className="border-b border-ink/10">
                ...
                {expanded && (
                  <div className="pb-3">
                    {(item[0] === "solutions"
                      ? [...data2_copy.items.filter((p: any) => p.key).map((p: any) => ({ name: p.name, href: `/why-lidespy/${p.key}` })), ...data.map((s: any) => ({ name: s[1], href: `/services/${s[0]}` }))]
                      : grids_new[item[0]].map((c: any) => ({ name: c[0], href: c[2], mediaKit: c[3] }))
                    ).map((l: any) => {
        ...
        <div className="pointer-events-none absolute inset-x-0 top-full hidden px-8 pb-6 lg:block">
          <div className="pointer-events-auto mx-auto grid max-w-[1280px] grid-cols-[minmax(260px,340px)_minmax(0,1fr)] overflow-hidden rounded-card bg-ink shadow-[0_40px_80px_-20px_rgba(18,21,15,0.45)]">
            <div className="flex flex-col px-[34px] py-9">
              <div className="mb-[26px] text-[11px] font-semibold tracking-[0.14em] text-cream/45 uppercase">{data2_copy.label}</div>
              {data2_copy.items.map((p: any) => (
      ...
      {toast && (
        <div role={toast.tone === "error" ? "alert" : "status"} className={`fixed inset-x-4 bottom-5 z-80 mx-auto max-w-[420px] rounded-card border bg-ink px-4 py-3.5 text-[12.5px] leading-[1.55] text-cream shadow-[0_12px_40px_rgba(0,0,0,0.28)] sm:right-5 sm:left-auto sm:mx-0 ${toast.tone === "error" ? "border-brand-cta/50" : "border-brand/60"}`}>
          <div className="flex items-start gap-3">
            ...
            <button type="button" onClick={() => setToast(null)} aria-label={COPY_FINAL.close} className="mt-[2px] cursor-pointer text-cream/55 transition-colors hover:text-cream">
      ...
      {isOpen3 && (
        <div role="dialog" aria-modal="true" aria-labelledby="optout-title" className="fixed inset-0 z-70 flex items-end justify-center bg-ink/55 p-4 backdrop-blur-[2px] sm:items-center" onMouseDown={(e) => { if (e.target === e.currentTarget && status !== "sending") close(); }}>
          <div className="relative w-full max-w-[440px] rounded-card border border-ink/12 bg-white p-[clamp(22px,3vw,32px)]">
            <button type="button" onClick={close} disabled={status === "sending"} aria-label={COPY_FINAL.close} className="absolute top-4 right-4 cursor-pointer rounded-ui p-1.5 text-muted-3 transition-colors hover:bg-panel hover:text-ink disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent">
              ...
              <form onSubmit={submit} noValidate className="flex flex-col gap-4">
                <div className="flex flex-col gap-2 pr-8">
                  <h2 id="optout-title" className="m-0 text-[20px] leading-[1.2] font-medium tracking-[-0.02em] text-ink">{COPY_FINAL.title}</h2>
                  <p className="m-0 text-[13px] leading-[1.6] text-muted-2 text-pretty">{COPY_FINAL.intro}</p>
                </div>
                <label className="flex flex-col gap-[7px]">
                  <span className="text-[12px] font-semibold text-ink">{COPY_FINAL.nameLabel}</span>
                  <input ref={firstFieldRef} name="fullName" type="text" autoComplete="name" maxLength={120} value={name} placeholder={COPY_FINAL.namePlaceholder} onChange={(e) => setName(e.target.value)} aria-invalid={fieldErrors.name ? true : undefined} className={`${inputCls} ${fieldErrors.name ? "border-brand-cta bg-brand-cta/5" : ""}`} />
                  {fieldErrors.name && (
                    <span className="text-[11.5px] leading-[1.45] text-brand-cta">{fieldErrors.name}</span>
                  )}
                </label>
                <label className="flex flex-col gap-[7px]">
                  <span className="text-[12px] font-semibold text-ink">{COPY_FINAL.emailLabel}</span>
                  <input name="email" type="email" autoComplete="email" maxLength={160} value={email} placeholder={COPY_FINAL.emailPlaceholder} onChange={(e) => setEmail(e.target.value)} aria-invalid={fieldErrors.email ? true : undefined} className={`${inputCls} ${fieldErrors.email ? "border-brand-cta bg-brand-cta/5" : ""}`} />
                  {fieldErrors.email && (
                ...
                <button type="submit" disabled={status === "sending"} className="mt-1 cursor-pointer rounded-ui bg-brand-cta px-6 py-4 text-[12px] font-semibold tracking-[0.04em] text-white uppercase transition-colors hover:bg-ink hover:text-coral disabled:cursor-not-allowed disabled:opacity-60">
                  {status === "sending" ? COPY_FINAL.submitting : COPY_FINAL.submit}
                </button>
              </form>
          </div>
        </div>
